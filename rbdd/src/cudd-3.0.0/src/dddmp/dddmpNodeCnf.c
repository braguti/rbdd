...
int DddmpClearVisitedCnfRecur(DdNode *f);
...
int DddmpDdNodesResetCountRecur(DdNode *f);
int DddmpDdNodesCountEdgesRecur(DdNode *f);
  ...
  )
{
  ...
  if (Cudd_IsComplement (f)) {
    fprintf (stdout, "sign=- ptr=%ud ", fPtr->index);
  } else {
    fprintf (stdout, "sign=+ ptr=%ud ", fPtr->index);
  }
  ...
  fprintf (stdout,  
    "thenPtr=%ud elsePtr=%ud BddId=%d CnfId=%d Visited=%d\n",
    fPtr->index, fPtr->index,
