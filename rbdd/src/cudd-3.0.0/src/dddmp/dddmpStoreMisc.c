  ...
  for (i=0; i<n; i++) {
    ...
    if (Cudd_IsComplement(f[i])) {
      ...
      retValue = fprintf (fp, "(NOT node%lx))\n",
        f[i]->index / (unsigned long) sizeof(DdNode));
#else
  ...
  )
{
  ...
  if (f == DD_ONE (ddMgr)) {
    ...
    retValue = fprintf (fp,
      "(OR node%lx vss vdd)\n",
      f->index / (unsigned long) sizeof(DdNode));
#else
    retValue = fprintf (fp,
      "(OR node%x vss vdd)\n",
  ...
  retValue = fprintf (fp, "node%lx) (AND (NOT ",
    T->index / (unsigned long) sizeof(DdNode));
#else
  ...
  if (Cudd_IsComplement(cuddE(f))) {
    retValue = fprintf (fp, ") (NOT node%lx)))\n",
      E->index / (unsigned long) sizeof(DdNode));
  } else {
    retValue = fprintf (fp, ") node%lx))\n",
      E->index / (unsigned long) sizeof(DdNode));
  }
  ...
  for (i = 0; i < n; i++) {
    if (outputNames == NULL) {
      retValue = fprintf(fp,
        ...
        f[i]->index / (unsigned long) sizeof(DdNode), i);
#else
...
#if SIZEOF_VOID_P == 8
        ".names node%lx %s\n",
        f[i]->index / (unsigned long) sizeof(DdNode), outputNames[i]);
#else
        ".names node%x %s\n",
