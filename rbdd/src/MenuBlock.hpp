...
#ifndef __MenuBlock__
#define __MenuBlock__
    ...
    template <class Q> void addIncrement(Q& var2pos, Q& pos2var, int delta) {
        ...
        for(int var : indices) {
            if (var2pos[var] + delta < 0) {
                Rcpp::Rcerr << "In menu " << description << " ";
                Rcpp::Rcerr << "Error addIncrement index " << var
                          << " from " << var2pos[var] << " to " << var2pos[var] + delta << std::endl;
                Rcpp::Rcerr << "delta : " << delta << std::endl;
                Rcpp::Rcerr << "positions: ";
                for(int x : indices){
                    Rcpp::Rcerr << var2pos[x] << " ";
                }
                Rcpp::Rcerr << std::endl;
