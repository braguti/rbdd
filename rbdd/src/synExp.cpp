...
void synCompound::print(std::ostream& os) const {
    ...
    if (Ce1 == NULL || Ce2 == NULL || Ce3 == NULL) {
        Rcpp::stop("Null pointer in SynCompound::print");
    }
...
std::list<synExp *> toCNF_rec(synExp* e) {
    ...
    if (e->isOr()) {
        ...
        for(synExp* a : resP) {
            for(synExp* b : resQ) {
                res.push_back(makeOr(a->copy_exp(),b->copy_exp()));
            }
        }
        for(synExp* a : resP)
            a->destroy();
    ...
    if (e->isNot()) {
        ...
        if (negated->isOr()) {
            std::list<synExp*> t = toCNF_rec(makeAnd(makeNot(negated->first()),
                                                     makeNot(negated->second())));
            res.insert(res.end(), t.begin(), t.end());
            delete negated;
            delete e;
    ...
    Rcpp::Rcerr << "Unhandled case in toCNF : " << e << std::endl;
    return res;
    //exit(-1);
}
    ...
    for(synExp* s : theList) {
        ...
        while (!stack.empty()) {
            ...
            if (trav->isOr()) {
                ...
                // Can be synModule
                //else {
                //    Rcpp::Rcerr << "Neither or nor literal " << trav << std::endl;
                //    exit(-1);
                //}
        }
        ...
        if (!isTrue) {
            if (! repetitions) result.push_back(s);
            else {
...
std::vector<int> synExp::toDimacs(std::map<std::string, int>& theMap) {
    ...
    Rcpp::stop("Unhandled case converting clause %s to DIMACS format", this);
...
void synSymbol::destroy() {
   // Rcpp::Rcerr << "Destroying " << (synExp*) this << std::endl;
        delete this;
}
